...
namespace Ody\Swoole\Declarative\Swoole;

use Closure;
use GraphQL\Error\FormattedError;
use GraphQL\Type\Schema;
use OutOfBoundsException;
use function Ody\Swoole\Declarative\set;
...
use Throwable;
use Ody\Swoole\Declarative\Container;
...
use const Ody\Swoole\Declarative\Route\DID_MATCH;
...
function emit(string $content, int $status = 200, array $headers = []): void
{
    ...
    \Ody\Swoole\Declarative\Swoole\response()->status($status);
...
function raw(): ?string
{
    ...
    $content = \Siler\Swoole\request()->rawContent();
...
function http_server_port(Server $server, callable $handler, int $port = 80, string $host = '0.0.0.0'): ServerPort
{
    ...
    $server_port = $server->addlistener($host, $port, (int)$sock_type);
...
function graphql_handler(Schema $schema, $root_value = null, $context = null): Closure
{
    return static function () use ($schema, $root_value, $context): void {
        ...
        try {
            \Siler\Swoole\json(execute($schema, graphql_request()->toArray(), $root_value, $context));
